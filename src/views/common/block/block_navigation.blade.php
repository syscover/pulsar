...
<ul id="nav">
    ...
    @if(Session::get('userAcl')->isAllowed(Auth::user()->profile_010, 'admin', 'access') && $aux[2]->active_012)
        <li{!! Miscellaneous::setCurrentOpenPage(['admin-user', 'admin-lang', 'admin-country', 'admin-country-at1', 'admin-country-at2', 'admin-country-at3', 'admin-mod', 'admin-cron', 'admin-pass-profile', 'admin-pass-resource', 'admin-pass-actions', 'admin-pass-pass']) !!}>
            ...
            <ul class="sub-menu">
                ...
                @if(Session::get('userAcl')->isAllowed(Auth::user()->profile_010, 'admin-mod', 'access'))
                        <li{!! Miscellaneous::setCurrentPage('admin-mod') !!}><a href="{{ url(config('pulsar.appName') . '/pulsar/packages') }}"><i class="cut-icon-grid"></i>Paquetes</a></li>
                @endif
                @if(Session::get('userAcl')->isAllowed(Auth::user()->profile_010, 'admin-cron', 'access'))
                        <li{!! Miscellaneous::setCurrentPage('admin-cron') !!}><a href="{{ url(config('pulsar.appName') . '/pulsar/cron/jobs') }}"><i class="icomoon-icon-stopwatch"></i>Tareas Cron</a></li>
                @endif
                @if(Session::get('userAcl')->isAllowed(Auth::user()->profile_010, 'admin-pass', 'access'))
                    <li{!! Miscellaneous::setOpenPage(['admin-pass-profile', 'admin-pass-resource', 'admin-pass-actions', 'admin-pass-profile', 'admin-pass-pass']) !!}>
                        <a href="javascript:void(0);"><i class="icon-user"></i>Permisos<span class="arrow"></span></a>
                        <ul class="sub-menu"{!! Miscellaneous::setDisplayPage(['admin-pass-resource', 'admin-pass-actions', 'admin-pass-profile', 'admin-pass-pass']) !!}>
                            @if(Session::get('userAcl')->isAllowed(Auth::user()->profile_010, 'admin-pass-profile', 'access'))
                                <li{!! Miscellaneous::setCurrentPage(array('admin-pass-profile', 'admin-pass-pass')) !!}><a href="{{ url(config('pulsar.appName') . '/pulsar/profiles') }}"><i class="icomoon-icon-users-2"></i>Perfiles</a></li>
                            @endif
