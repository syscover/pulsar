<div class="form-group">
    ...
    <div class="col-md-{{ $sizeField or 10 }}">
        <input class="form-control{{ isset($required)? ' required' : null }}" type="text" name="{{ $name }}" value="{{ isset($value)? $value : null }}" @if(isset($maxLength)) maxlength="{{ $maxLength }}"@endif @if(isset($rangeLength))rangelength="{{ $rangeLength }}"@endif @if(isset($min)) min="{{ $min }}"@endif{{ isset($readOnly) && $readOnly? ' readonly' : null }}>
