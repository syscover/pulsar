<div @if(isset($containerId)) id="{{ $containerId }}"@endif class="form-group">
    ...
    <div class="col-md-{{ $fieldSize or 10 }}">
        <select{!! isset($id)? ' id="' . $id . '" ' : null !!}{!! isset($class)? ' class="' . $class . '"' : null !!} {!! isset($style)? ' style="' . $style . '" ' : null !!}name="{{ $name }}"{{ isset($multiple)? ' multiple' : null }}{{ isset($required) && $required? ' required' : null }}{{ isset($disabled) && $disabled? ' disabled' : null }}{!! isset($data)? Miscellaneous::setDataAttributes($data) : null !!}>
