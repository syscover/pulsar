<div{!! isset($wrapperId)? ' id="' . $wrapperId . '"' : null !!} class="form-group">
    ...
    <div class="col-md-{{ $fieldSize or 2 }}">
        <select{!! isset($id)? ' id="' . $id . '" ' : null !!} class="col-md-12{{ isset($required) && $required? ' required' : null }}{{ isset($class)? ' ' . $class : null }}" {!! isset($style)? ' style="' . $style . '" ' : null !!}name="{{ $name }}"{{ isset($multiple)? ' multiple' : null }}{!! isset($data)? Miscellaneous::setDataAttributes($data) : null !!}>
