...
(function ($) {
  ...
  $.FroalaEditor.PLUGINS.link = function (editor) {
    ...
    function _hideInsertPopup () {
      if (!get() && editor.$el.find('.fr-marker').length) {
        editor.events.focus();
    ...
    function _init () {
      editor.events.on('mouseup', _edit);
      editor.events.on('touchend', _edit);
    ...
    function insert (href, text, attrs) {
      ...
      // Add autoprefix.
      if (href.indexOf('mailto:') !== 0 && href.indexOf('data:image') !== 0 && editor.opts.linkAutoPrefix !== '' && !/^(https?:|ftps?:|)\/\//.test(href)) {
  ...
  $.FroalaEditor.RegisterCommand('linkStyle', {
    ...
    html: function () {
      ...
      for (var cls in options) {
        c += '<li><a class="fr-command" data-cmd="linkStyle" data-param1="' + cls + '">' + options[cls] + '</a></li>';
      }
      c += '</ul>';
